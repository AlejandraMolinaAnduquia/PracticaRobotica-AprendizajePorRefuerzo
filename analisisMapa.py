...
def mover_robot(tablaQ, cell_index, x, y, num_filas, num_columnas):
    ...
    """
    # Determinar la acción basada en la tabla Q
    politica = tablaQ.get(cell_index, [0, 0, 0, 0])  # Obtener la política actual
    accion = np.argmax(politica)  # Acción con mayor probabilidad
    ...
    if accion == 0 and y > 0:  # Izquierda
        comunicacionArduino.send_command('w')  # Comando Arduino
        y -= 1
    elif accion == 1 and x > 0:  # Arriba
        comunicacionArduino.send_command('s')  # Comando Arduino
        x -= 1
    elif accion == 2 and y < num_columnas - 1:  # Derecha
        comunicacionArduino.send_command('a')  # Comando Arduino
        y += 1
    elif accion == 3 and x < num_filas - 1:  # Abajo
        comunicacionArduino.send_command('d')  # Comando Arduino
        x += 1
    ...
    if politica[accion] == 0:
        print("Movimiento no permitido, recalibrando...")
        comunicacionArduino.send_command('x')  # Detener el robot
        return {
            "x_inicial": x_inicial,
            "y_inicial": y_inicial,
            "x_final": x_inicial,
            "y_final": y_inicial,
            "nuevo_cell_index": cell_index,
            "movimiento": None
        }

    # Movimiento válido
    print(f"Robot movido de ({x_inicial}, {y_inicial}) a ({x}, {y})")
    return {
        "x_inicial": x_inicial,
        "y_inicial": y_inicial,
        "x_final": x,
        "y_final": y,
        "nuevo_cell_index": nuevo_cell_index,
        "movimiento": accion
    }
